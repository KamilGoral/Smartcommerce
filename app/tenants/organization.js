    ...
    request.onload = function () {
      ...
      if (
        ...
        var tableUsers = $("#table_users_list").DataTable({
          ...
          destroy: true,
          orderMulti: true,
          order: [[4, "desc"]], // status przesunięty o 1 po dodaniu kolumny imię
          ...
          columns: [
            ...
            {
              // name + familyName z users GET (nowe pola). Wymaga dodatkowego <th> w Webflow.
              orderable: true,
              data: null,
              render: function (data, type, row) {
                const full = [row.name, row.familyName]
                  .filter(Boolean)
                  .join(" ");
                return full || "—";
              },
      ...
      var toParse = items;
        ...
        $("#table_wholesalers_list").DataTable({
          ...
          initComplete: function () {
            var api = this.api();
            $("#table_wholesalers_list_wrapper")
              .off("keyup.wsSearch input.wsSearch search.wsSearch")
              .on(
                "keyup.wsSearch input.wsSearch search.wsSearch",
                "input[type='search']",
                function () {
        ...
        $("#table_wholesalers_list_bonus").DataTable({
          ...
          initComplete: function () {
            var api = this.api();
            $("#table_wholesalers_list_bonus_wrapper")
              .off("keyup.wsSearch input.wsSearch search.wsSearch")
              .on(
                "keyup.wsSearch input.wsSearch search.wsSearch",
                "input[type='search']",
