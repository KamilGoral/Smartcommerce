  ...
  function getShop() {
    ...
    request.onload = function () {
      ...
      if (request.status >= 200 && request.status < 400) {
        ...
        document.querySelector('[shopdata="shopName"]').textContent = data.name || 'N/A';
        document.querySelector('[shopdata="shopKey"]').textContent = data.shopKey || 'N/A';

        // Address information
        if (data.address) {
          const { country, line1, town, state, postcode } = data.address;
          const addressDescription = `${country}, ${line1}, ${town}, ${state}, ${postcode}`;
          document.querySelector('[shopdata="address"]').textContent = addressDescription || 'N/A';
        }

        // Email information
        if (data.emails && data.emails.length > 0) {
          const emailsDescription = data.emails.map(email => email.email).join(", ");
          document.querySelector('[shopdata="emails"]').textContent = emailsDescription || 'N/A';
        }

        // Phone information
        if (data.phones && data.phones.length > 0) {
          const phonesDescription = data.phones.map(phone => phone.phone).join(", ");
          document.querySelector('[shopdata="phones"]').textContent = phonesDescription || 'N/A';
        }
        if (data.merchantConsoleShopId) {
        document.querySelector('[shopdata="merchantConsoleShopId"]').textContent = data.merchantConsoleShopId || "";
        }
