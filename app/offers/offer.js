...
docReady(function () {
    ...
    ShopBread.setAttribute("href", "https://" + DomainName + "/app/shops/shop?shopKey=" + shopKey);    
    ...
    function getProductDetails(rowData) {
        let url = new URL(InvokeURL + 'shops/' + shopKey + '/products/' + rowData.ean);
    ...
    function getProductHistory(rowData) {
        ...
        let url = new URL(InvokeURL + 'shops/' + shopKey + '/products/' + rowData.ean + '/history?perPage=91&page=1');
        let request = new XMLHttpRequest();
        request.open('GET', url, true);
        request.setRequestHeader("Authorization", orgToken);
        request.onload = function () {
            ...
            if (request.status >= 200 && request.status < 400) {
                ...
                const pRetailPriceChange = document.getElementById("pRetailPriceChange");
                pRetailPriceChange.textContent = "(" + displayData(parseFloat(((dataToChart.retailPrice[0] - dataToChart.retailPrice.slice(-1)[0]) / dataToChart.retailPrice.slice(-1)[0]) * 100).toFixed(2)) + "%)";
                const pStandardPriceChange = document.getElementById("pStandardPriceChange");
                pStandardPriceChange.textContent = "(" + displayData(parseFloat(((dataToChart.standardPrice[0] - dataToChart.standardPrice.slice(-1)[0]) / dataToChart.standardPrice.slice(-1)[0]) * 100).toFixed(2)) + "%)";
                const pSales7 = document.getElementById("pSales7");
                pSales7.textContent = displayData(dataToChart.volume.slice(0, 7).reduce((a, b) => a + b, 0));
                const pStockDays = document.getElementById("pStockDays");
                pStockDays.textContent = displayData(Math.round(rowData.inStock.value / dataToChart.volume.slice(0, 7).reduce((a, b) => a + b, 0) * 7));
                const pSales90 = document.getElementById("pSales90");
                pSales90.textContent = displayData(dataToChart.volume.slice(0, 90).reduce((a, b) => a + b, 0));
                const pSales90Change = document.getElementById("pSales90Change");
                pSales90Change.textContent = "(" + displayData(parseFloat((dataToChart.volume.slice(-90).reduce((a, b) => a + b, 0) - dataToChart.volume.slice(0, 90).reduce((a, b) => a + b, 0)) / dataToChart.volume.slice(0, 90).reduce((a, b) => a + b, 0) * 100).toFixed(2)) + "%)"


                var scaleMax = Math.max.apply(Math, [...dataToChart.highest, ...dataToChart.retailPrice]) * 1.1;
                var scaleMin = Math.min.apply(Math, [...dataToChart.lowest, ...dataToChart.standardPrice]) * 0.9;
                ...
                var options = {
                    ...
                    colors: ['#FD6A6A', '#F9C80E', '#4CAF50', '#3F51B5', '#03A9F4', '#92A9BD', '#D3DEDC'],
                    title: {
    ...
    function format(d) {
        ...
        function myFunction(item) {
            ...
            var tableRowHtml = '<tr>' + '<td>' + item.wholesalerKey + '</td>' + '<td>' + item.netPrice + '</td>' + '<td>' + item.netNetPrice + '</td>' + '<td>' + item.set + '</td>' + '<td>' + typeOfSource + '</td>';
            var typeOfPromotion = ''
            if (item.promotion != null) {
                ...
                tableRowHtml += '<td>' + typeOfPromotion + '</td>' + '<td>' + item.promotion.worthThreshold + '</td>' + '<td>' + item.promotion.quantityThreshold + '</td>' + '<td>' + item.promotion.package + '</td>' + '</tr>'
            } else {
                tableRowHtml += '<td>' + '</td>' + '<td>' + '</td>' + '<td>' + '</td>' + '<td>' + '</td>' + '<td>' + '</td>' + '</tr>'
            }
            toDisplayHtml += tableRowHtml;
        }
        arr.forEach(myFunction);
        return '<table><tr><th>Dostawca</th><th>Cena net</th><th>Cena netnet</th><th>Paczka</th><th>Zrodlo</th><th>Promocja</th><th>Typ</th><th>Prog zlotowkowy</th><th>Prog ilosciowy</th><th>Opakowanie</th></tr>' + toDisplayHtml + '</table>'
    }
    ...
    var table = $('#table_id').DataTable({
        ...
        ajax: function (data, callback, settings) {
            ...
            var QStr = "?perPage=" + data.length + "&page=" + (data.start + data.length) / data.length
            ...
            var rotIndi = $('#rotationIndicator').map(function () {
                return this.value;
            }).get();
            ...
            var whKeyIndi = $('#wholesalerKeyIndicator').map(function () {
                return this.value;
            }).get();
        ...
        },
        {
            ...
            "data": null,
            "defaultContent": "<img src='https://uploads-ssl.webflow.com/6041108bece36760b4e14016/6240120504eebc8de2698a1f_panel.svg' alt='details'></img>"
        },
        {
            "orderable": false,
            "data": "name"
        ...
        },
        {
            ...
            "data": "standardPrice",
            "render": function (data) {
                if (data !== null && data.hasOwnProperty("premium") && data.premium !== null) {
                    if (data.premium >= 0) {
        ...
        {//Tutaj beda promocje jako obrazki renderowane
            ...
            "render": function (data) {
                if (data !== null && data.netPrice !== null) {
                    var mysorteddata = data.sort((a, b) => (a.netPrice > b.netPrice) && 1 || -1);
