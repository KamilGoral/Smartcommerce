    ...
    searchIDs.forEach((wholesaler) => {
      $("#DeletedContainer").append(`
        <div class="deletedwh" id="d${wholesaler}">
          ...
          >
            <span class="icon initial" style="maring-left:6px;margin-left: 6px;margin-top: 1px;"></span>
            <span class="icon loading" style="maring-left:6px;margin-left: 6px;margin-top: 1px;"></span>
            <span class="icon final" style="maring-left:6px;margin-left: 6px;margin-top: 1px;"></span>
          </label>
        </div>
      `);
    });
  ...
  function GetSplittedProducts() {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        // Jeśli przyszła pusta tablica, nadal pokazujemy tabelę z pustym stanem
        if (!resultProducts.items || resultProducts.items.length === 0) {
          resultProducts.items = [];
        }

        // Zapis do sesji + odtworzenie inputów z localStorage
