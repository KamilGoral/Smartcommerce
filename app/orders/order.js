...
docReady(function () {
  ...
  function GetSplittedProducts() {
    let resultProducts = { items: [] }; // domyślna wartość

    // Sprawdzamy, czy tabela ma już dane
    if ($.fn.DataTable.isDataTable("#spl_table")) {
      const table = $("#spl_table").DataTable();
      if (table.data().length > 0) {
        console.log(
          "Dane już zostały załadowane, przechodzę do ustawiania filtrów."
        );
        applyFiltersToTable(); // Funkcja ustawia filtry, jeśli dane już są załadowane
        return;
      }
    }
    ...
    $.ajax({
      ...
      complete: function () {
        ...
        var table = $("#spl_table").DataTable({
          ...
          buttons: [
            ...
            {
              text: '<img src="https://uploads-ssl.webflow.com/6041108bece36760b4e14016/65e83bbae9eb38d00e79cb7d9_collapse-all.svg" alt="collapse-all">',
          ...
          columns: [
            { data: "name" },
            { data: "countryDistributorName", defaultContent: "-" },
            { data: "gtin" },
            { data: "stock" },
            { data: "quantity" },
            { data: "standardPrice" },
          ...
          initComplete: function (settings, json) {
            // Ustawienie filtrów tylko jeśli dane zostały załadowane po raz pierwszy
            applyFiltersToTable();
          },
        });
      },
      success: function (response) {
        ...
        // Zapisujemy dane do sesji + odtwarzamy dane z localStorage
      ...
      error: function (jqXHR, exception) {
        ...
        resultProducts = { items: [] }; // Jeśli wystąpi błąd, ustawiamy pustą tablicę
        $("#splittedProductsSection").show();
      },
    });
  }
  ...
  function applyFiltersToTable() {
    ...
    // Zastosowanie filtrów do tabeli (filtracja na podstawie wartości wybranych w dropdown)
    const table = $("#spl_table").DataTable();
    table.rows().every(function () {
      const rowData = this.data();
      const wholesalerValue = $(this.node()).find("select").val();
      const rotationValue = $(this.node()).find("td:eq(12)").text().trim();
      ...
      if (matchWholesaler && matchRotation) {
        this.show();
      } else {
        this.hide();
      }
